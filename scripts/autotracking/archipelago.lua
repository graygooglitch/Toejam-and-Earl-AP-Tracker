...
function incrementItem(item_code, item_type)
	...
	if obj then
		...
		if item_code == "progkey" then
			local progkey = Tracker:FindObjectForCode("progkey")
			for i=2,KEY_LEVEL_ACCESS[progkey.AcquiredCount+1],1 do
				local obj = Tracker:FindObjectForCode(string.format("lat%s",i))
				obj.Active = true
			end
			local elevator = Tracker:FindObjectForCode("elevator")
			elevator.CurrentStage = KEY_LEVEL_ACCESS[progkey.AcquiredCount+1]-1
		end
	elseif AUTOTRACKER_ENABLE_DEBUG_LOGGING_AP then
...
function apply_slot_data(slot_data)
	...
	for i, itemcount in ipairs(items_per_level) do
		local obj = Tracker:FindObjectForCode(string.format("@Main/Level %s/Items",i-1))
		obj.AvailableChestCount = itemcount
	end
